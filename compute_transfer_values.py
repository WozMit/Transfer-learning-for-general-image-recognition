...
def compute_batch_test_transfer_values(dataset_dir, list_filenames):
	...
	if model == "vgg16":
		...
		for i in range(nimages):
			transfer_values_test[i] = models_test.inception_resnet_v2_transfer_values(image=images_test[i])
...
def main():
	...
	elif model_type == "inceptresv2":
		transfer_len = models_test.inception_resnet_v2_transfer_len()
		print("transfer_len {}".format(transfer_len))
		model = model_type
		standard_size = 299
