...
def compute_color_transformations(pil_pad_img, cls, offset_index_image, images_dataset, cls_dataset):
	...
	for i in range(num_color_transf):
...
def compute_batch_train_transfer_values(dataset_dir, list_filenames):
	...
	for i in range(final_nimages):
		transfer_values_train[i] = model.transfer_values(image=images_train[i])
	# end time
	end_time = time.time()
...
def compute_batch_test_transfer_values(dataset_dir, list_filenames):
	...
	for i in range(nimages):
		transfer_values_test[i] = model.transfer_values(image=images_test[i])
	# end time
	end_time = time.time()
	time_dif = end_time - start_time
	print("Time usage: " + str(timedelta(seconds=int(round(time_dif)))))
...
def main():
	global session, image, wrap_pre_process_image, model, transfer_len
